...
const hotspots = [
    ...
    { id: 'sofaSheet', image: sofaSheetImg, hoverPath: 'M76,75 L95,75 L95,95 L76,95 Z' },
...
const InteractiveHotspotLayer = ({ isMobile, isMobileLandscape }) => {
    ...
    return (
        ...
        >
            ...
            {/* Mobile Tutorial Hint (Pulses initially, fades out on touch exploration) */}
            <AnimatePresence>
                {isMobile && !tappedId && (
                    <motion.div
                        initial={{ opacity: 0, y: 10 }}
                    ...
                    >
                        ...
                        <span className="text-[10px] font-extrabold tracking-[0.2em] text-black/60 uppercase font-['Outfit']">
                            Touch drawing to explore
                        </span>
                    </motion.div>
                )}
            </AnimatePresence>
...
const IntroSequence = () => {
    ...
    // Desktop/Tablet-only headline opacity transform (declared at top level to comply with React's Rules of Hooks)
    const desktopHeadlineOpacity = useTransform(progress, [0.01, 0.04], [0, 1]);

    // ── Mobile growth and motion transforms (Cinematic Scroll Zoom) ──
    const mobileSketchOpacity = useTransform(progress, [0, 0.7, 0.92], [1, 0.7, 0]);
    const mobileLogoScale = useTransform(progress, [0, 0.65], [1, 1.7]);
    const mobileLogoY = useTransform(progress, [0, 0.65], [0, 160]);
    const mobileLogoOpacity = useTransform(progress, [0, 0.7, 0.92], [1, 1, 0]);

    const mobileHeadlineScale = useTransform(progress, [0, 0.65], [1, 1.25]);
    const mobileHeadlineY = useTransform(progress, [0, 0.65], [0, -130]);
        ...
        <section ref={containerRef} className={`relative ${isMobile ? 'h-screen' : 'h-[115vh]'} w-full bg-white z-[60]`}>
            ...
            <div className={`${isMobile ? 'relative h-full' : 'sticky top-0 h-screen'} w-full overflow-hidden z-[60] bg-white flex flex-col items-center justify-center transform-gpu`}>
                ...
                {isMobile && !isMobileLandscape && (
                    ...
                    >
                        <h1 className="text-xl sm:text-2xl font-black text-black leading-snug tracking-wider text-center uppercase font-['Outfit']">
