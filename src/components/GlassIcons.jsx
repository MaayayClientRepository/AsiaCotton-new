...
const GlassIcons = ({ items, className, onHover }) => {
    ...
    return (
        <div className={`grid gap-[3em] lg:gap-[5em] grid-cols-2 lg:grid-cols-2 mx-auto py-[3em] overflow-visible ${className || ''}`}>
            {items.map((item, index) => (
                ...
                >
                    ...
                    <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-sm opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(20%)] font-bold text-white/80 tracking-widest uppercase">
                        {item.label}
                    </span>
                </button>
            ))}
