...
const Navbar = ({ isHome }) => {
    ...
    return (
        <div className={cn(
            ...
            <div className="flex items-center gap-2 bg-[#111111] p-2 rounded-2xl shadow-2xl border border-white/10 backdrop-blur-md">
                {/* Logo Section */}
                <a
                    href="/"
                    className="flex items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-white rounded-full transition-transform hover:scale-105 active:scale-95 shrink-0 shadow-lg"
                >
                    <img 
                        src={logo} 
                        alt="Asia Cotton" 
                    ...
                    {links.map((link, idx) => (
                        ...
                        >
                            {React.cloneElement(link.icon, { 
                                className: "w-6 h-6 md:w-6 md:h-6 text-white/70 group-hover:text-white transition-colors" 
                            })}
                        </a>
                    ))}
                </div>
